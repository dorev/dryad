...
struct Pitch
{
  ...
  {
    if (!(!strcmp(noteNode.name(), "note") && noteNode.child("pitch") && noteNode.child("duration")))
      return false;
  ...
  bool isValid()
  {
    ...
    return noteNum(_step) >= 0 && _octave >= 0 && _duration > 0 && _measure >= 0 && _num == _octave * 12 + (noteNum(_step) + _alter) && _nodePtr != nullptr;
  }
  ...
  bool operator<(const Pitch &other) const
  {
    return _num < other._num || (_num == other._num && _duration < other._duration);
  }
};
